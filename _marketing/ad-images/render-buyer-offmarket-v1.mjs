#!/usr/bin/env node
/**
 * Buyer-side single-image ad, v1.
 *
 * "The best deals never hit MLS." — Off-Market Buyer Network angle.
 * Routes to /get-deals?cat=cash on resolveproperty.ca.
 *
 * Style: matches the Resolve seller ads (stone bg, navy type, bronze
 * accent, Newsreader + Inter, HomeLife G1 Realty Inc., Brokerage in
 * the footer) so the visual language stays consistent for the ad
 * account. Eyebrow flips from "GTA · Seller Representation" to
 * "GTA · Real Estate" so the ad reads as buyer-appropriate without
 * making Resolve sound seller-only.
 *
 * Compliance:
 *   - No "exclusive network" language (RECO Bulletin 5.3)
 *   - No dollar figures, no guarantees, no urgency claims
 *   - Brokerage attribution present per RECO 5.1
 *   - Fits HOUSING Special Ad Category (no personal-attribute claims)
 *
 * Output: public/ads/buyer-offmarket-v1.png  (1080×1350, 4:5 Feed)
 */
    ...
    <div class="footer">
      <div>
        <div>
          ...
          <div class="wordmark__descriptor">Buyer Representation</div>
        </div>
      ...
      <div class="footer-right">
        <div class="url">resolveproperty.ca/get-deals</div>
...
async function run() {
  ...
  console.log('[buyer-offmarket-v1] launching puppeteer')
  ...
  await browser.close()
  console.log('[buyer-offmarket-v1] done')
}
